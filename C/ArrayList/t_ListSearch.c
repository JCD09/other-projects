...
int f_findInt(void* examInt, void* targetInt){
  int* examint = examInt;
  int* targetint = targetInt;
  //printf("printing examInt and target int %d %d\n",*examint, *targetint );

  if(*examint == *targetint){return 1;}
  else{return 0; }
}
...
int main(){
  ...
  printf("Freeing list\n");
  printf("Trying to find variable 8 in the list\n");
  int var8 = 8;
  int* item = ListSearch(listOne,&f_findInt,&var8);
  assert(*item = 8);
  printf("Value 8 successfully found\n");
  printf("Trying to find variable 8 in the list\n");
  int var11 = 11;
  item = ListSearch(listOne,&f_findInt,&var11);
  assert(item == NULL);
  printf("var 11 not fouund, returned pointer is null\n");
